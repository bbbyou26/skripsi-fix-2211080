...
from flask import render_template, redirect, url_for, session, request, jsonify, send_from_directory
from werkzeug.utils import secure_filename
from werkzeug.security import generate_password_hash, check_password_hash

from config import app, users_collection, permission_requests_collection, creation_requests_collection
...
# ---------------------------------------------------------------
# STATIC ASSETS ROUTING UNTUK WEB-APP
# ---------------------------------------------------------------
@app.route("/style-css/<path:filename>")
def serve_web_app_css(filename):
    return send_from_directory(os.path.join(app.root_path, "web-app", "style-css"), filename)


@app.route("/engine-js/<path:filename>")
def serve_web_app_js(filename):
    return send_from_directory(os.path.join(app.root_path, "web-app", "engine-js"), filename)


@app.route("/img/<path:filename>")
def serve_web_app_img(filename):
    return send_from_directory(os.path.join(app.root_path, "web-app", "img"), filename)


@app.route("/web-app/<path:filename>")
def serve_web_app_all(filename):
    return send_from_directory(os.path.join(app.root_path, "web-app"), filename)


# ---------------------------------------------------------------
# HALAMAN UTAMA (LANDING PAGE & TENTANG)
# ---------------------------------------------------------------
@app.route("/")
@app.route("/home")
@app.route("/home.html")
...
def tentang_page():
    user_data = get_current_user_data()
    return render_template("tentang.html", user=user_data)


# ---------------------------------------------------------------
# HALAMAN MAP & GIS APPLICATION
# ---------------------------------------------------------------
@app.route("/map.html")
def map_html_redirect():
    return redirect(url_for("app_page", **request.args), code=301)
...
def app_page():
    ...
    session.pop("spatial_chat_history", None)
...
# ---------------------------------------------------------------
@app.route("/auth.html")
def auth_html_redirect():
    return redirect(url_for("auth_page", **request.args), code=301)
...
@app.route("/api/permission/reject", methods=["POST"])
...
@app.route("/api/permission/request_creation", methods=["POST"])
def request_creation_quota():
    nama_akun = session.get("user")
    if not nama_akun:
        return jsonify({"error": "Unauthorized"}), 401
    ...
    creation_requests_collection.update_one(
        {"nama_akun": nama_akun, "status": "pending"},
        {
            "$set": {
                "nama_akun": nama_akun,
                "nama": user_doc.get("nama", nama_akun),
...
@app.route("/api/permission/creation_requests", methods=["GET"])
def get_creation_requests():
    ...
    requests_cursor = creation_requests_collection.find({"status": "pending"})
    requests_list = []
    for req in requests_cursor:
        requests_list.append({
            "nama_akun": req["nama_akun"],
...
@app.route("/api/permission/approve_creation", methods=["POST"])
def approve_creation_quota():
    nama_akun_session = session.get("user", "")
    is_admin = nama_akun_session.endswith(':admin') or nama_akun_session.endswith(':admin@2211080.com')
    if not is_admin:
        return jsonify({"error": "Unauthorized"}), 403
    ...
    if not target_nama_akun:
        return jsonify({"error": "Missing nama_akun"}), 400

    creation_requests_collection.update_one(
        {"nama_akun": target_nama_akun, "status": "pending"},
        {"$set": {"status": "approved"}}
    )
    users_collection.update_one(
        {"nama_akun": target_nama_akun},
        {"$inc": {"extra_creation_quota": jumlah}}
    )
    return jsonify({"success": True})


@app.route("/api/permission/reject_creation", methods=["POST"])
def reject_creation_quota():
    nama_akun_session = session.get("user", "")
    is_admin = nama_akun_session.endswith(':admin') or nama_akun_session.endswith(':admin@2211080.com')
    if not is_admin:
        return jsonify({"error": "Unauthorized"}), 403
...
def get_user_permissions():
    nama_akun = session.get("user")
    if not nama_akun:
        return jsonify({"permissions": [], "can_create": False, "creation_quota": 1})

    try:
        approved_requests = list(permission_requests_collection.find({
            "nama_akun": nama_akun,
            "status": "approved"
        }))
        actor_ids = [req["actor_id"] for req in approved_requests if "actor_id" in req]

        user_doc = users_collection.find_one({"nama_akun": nama_akun})
        extra_quota = user_doc.get("extra_creation_quota", 0) if user_doc else 0
        can_create = user_doc.get("can_create_markers", True) if user_doc else True

        return jsonify({
            "permissions": actor_ids,
            "can_create": can_create,
            "creation_quota": 1 + extra_quota
        })
    except Exception as e:
        print(f"[routes_auth] MongoDB connection notice: {e}")
        return jsonify({
            "permissions": [],
            "can_create": True,
            "creation_quota": 1
