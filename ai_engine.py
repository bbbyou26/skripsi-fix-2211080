...
def get_embedding(text: str) -> list:
    """Buat vector embedding dari teks menggunakan OpenAI Embeddings API (1024 dim untuk Pinecone)."""
    try:
        response = client_embed.embeddings.create(model=EMBED_MODEL, input=text, dimensions=1024)
        return response.data[0].embedding
    except Exception as e:
        # Fallback tanpa parameter dimensions jika API provider tidak mendukung parameter dimensions
        try:
            response = client_embed.embeddings.create(model=EMBED_MODEL, input=text)
            return response.data[0].embedding
        except Exception as e2:
            print(f"[ai_engine] Embedding failed: {e2}")
            return None
...
# PANGGILAN LLM 
# ===============================
def call_llm(system_prompt: str, user_prompt: str, temperature: float = 0.75, media_data: str = None, max_tokens: int = 4096) -> str:
