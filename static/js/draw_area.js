...
function finishDrawingLokasi() {
    ...
    polygon.isUserCreated = true;
    polygon.createdBy = window.currentUser;
...
function setupPolygonInteractions(polygon, isRestore = false) {
    if (!polygon.actorData) {
        polygon.actorData = {
            ...
            "Catatan": "",
            "Deskripsi": "",
            "createdBy": polygon.createdBy || window.currentUser,
            "isUserCreated": true
        };
    } else {
        if (polygon.createdBy) polygon.actorData.createdBy = polygon.createdBy;
        polygon.actorData.isUserCreated = true;
    }
    ...
    window.confirmLocation = function () {
        if (window.draggingMarker instanceof L.Polygon) {
            ...
            if (typeof updateMarkerCoords === 'function') {
                updateMarkerCoords(window.draggingMarker);
            }
            if (window.saveToNeo4j) {
                window.saveToNeo4j(window.draggingMarker);
            }
